...
- 后台进程查询加入覆盖输出收集的超时，避免 Wine 子进程持有输出管道时阻塞退出。
- Wine 服务无响应或已退出时，按运行库路径、容器路径和进程启动时间识别并清理遗留进程，保留其他容器与其他运行库的进程。
- 包含此前的 Steam UDP TOS 兼容补丁与麦克风权限声明。以完整运行库内测包分发。

## 0.5.6

- 为游戏内语音补齐 macOS 麦克风用途说明与音频输入 entitlement，修复《渔力全开》创建房间时因缺少隐私声明而停在 Wine 音频初始化的问题。麦克风授权仍由用户在系统提示中选择。

## 0.5.5

- 修复 Wine 在 macOS 上丢弃 IP_RECVTOS 辅助数据的问题；《渔力全开》的 Steam 网络初始化因此不再触发断言并卡住主菜单。
- 增加 UDP TOS 回归测试和可重现的 Wine 源码补丁；保留 MSync、多线程渲染及现有输入适配。

## 0.5.4

- 单击选中在鼠标按下时立即响应，不再等待双击识别超时；启动仍仅由双击触发。
- 《渔力全开》启动前校验并启用已验证的鼠标兼容组件，修复 AppDefaults 将 version.dll 强制设为 builtin 后鼠标失效的问题；遇到未知同名 DLL 时保留文件并提示。

## 0.5.3

- 游戏卡片改为单击选中、双击启动；移除卡片、详情栏及右键菜单的运行入口，避免浏览游戏时误启动。
